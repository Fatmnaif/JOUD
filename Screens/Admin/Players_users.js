...
export default function Player({route,navigation}) {
 ...
 const [playerv,setplayerv]= useState([]);
...
async function setfilterd(){
  ...
  setplayerv(querySnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })))
...
const onSearch=(text)=>{
   ...
   {
     deleteDoc(doc(db, "player", id)) .then(() => {
      alert("تم حذف لاعبة ")
    })
    .catch((error) => {
      alert(error.message)
...
return (
    <View style={styles.container}>
      ...
      <Text style={styles.Pagtitle}> اللاعبات  </Text>
