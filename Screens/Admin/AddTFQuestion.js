...
import { StyleSheet, Text,TextInput, View,TouchableOpacity,ScrollView,StatusBar,Image,SafeAreaView, Alert } from 'react-native';
...
export default function MultipleChoiceQuestion({navigation}) {
 //store user data
  ...
  const [Choices, setChoices] = useState(['صح','خطأ']);
...
if(level == 3){
 ...
 if(Questionis==""||Choices[0]==''||Choices[1]==''){Alert.alert("الرجاء إدخال كافة البيانات")}
 else{
   addDoc( collection(db,lvl),{
    Questionis: Questionis,
    Choices:[{label:Choices[0]},{label:Choices[1]}],
    Answer: answer,
    Point: points,
    QuestionLevel:level,
    Type:"صح وخطأ",
...
<View style={styles.container}>
              ...
              <Text style={styles.pagetitle}> إضافة صح و خطأ </Text>
              ...
              <Text style={styles.title3}> مستوى صعوبةالسؤال :</Text>
              <Text style={styles.title4}> النقاط المستحقة :</Text>
              <Text style={styles.title5}> </Text>
              <Text style={styles.title6}> </Text>
              <Text style={styles.title7}> </Text>
              <Text style={styles.title8}> </Text>
